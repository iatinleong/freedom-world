        ...
        {isGameStarted && (
          <header className="shrink-0 grid grid-cols-[1fr_auto_1fr] items-center px-3 sm:px-6 py-2 sm:py-3 border-b border-wuxia-gold/20 bg-gradient-to-b from-black/90 to-black/60 backdrop-blur-sm paper-edge z-50">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 justify-end">
              <Link href="/store" className="relative px-2 sm:px-3 h-8 sm:h-9 bg-wuxia-gold/10 border border-wuxia-gold/50 rounded-sm flex items-center justify-center hover:bg-wuxia-gold/20 transition-all group overflow-hidden" title="儲值 (開發測試中)">
                <div className="absolute inset-0 bg-wuxia-gold/20 animate-pulse"></div>
                <span className="text-xs sm:text-sm font-bold text-wuxia-gold relative z-10 drop-shadow-md whitespace-nowrap">黑市商人</span>
              </Link>
              <button onClick={() => setCharacterPanelOpen(true)} className="w-8 h-8 sm:w-9 sm:h-9 bg-black/60 border border-wuxia-gold/30 rounded-sm flex items-center justify-center hover:bg-wuxia-gold/10 hover:border-wuxia-gold/60 transition-all group" title="角色狀態與包裹">
                <User className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-wuxia-gold/70 group-hover:text-wuxia-gold" />
              </button>
              <button onClick={() => setQuestPanelOpen(true)} className="relative w-8 h-8 sm:w-9 sm:h-9 bg-black/60 border border-wuxia-gold/30 rounded-sm flex items-center justify-center hover:bg-wuxia-gold/10 hover:border-wuxia-gold/60 transition-all group" title="江湖傳聞 (任務)">
                <Scroll className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-wuxia-gold/70 group-hover:text-wuxia-gold" />
              </button>
              <button onClick={() => setGameMenuOpen(true)} className="w-8 h-8 sm:w-9 sm:h-9 bg-black/60 border border-wuxia-gold/30 rounded-sm flex items-center justify-center hover:bg-wuxia-gold/10 hover:border-wuxia-gold/60 transition-all group" title="系統選單">
