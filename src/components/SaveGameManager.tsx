...
import { Save, Trash2, Download, Upload, X, Clock, MapPin, TrendingUp } from 'lucide-react';
...
export function SaveGameManager({ isOpen, onClose }: SaveGameManagerProps) {
    ...
    // Fix: Get individual pieces of state to avoid infinite loop
    ...
    }), [player, world, system, narrative, options, summary, isGameStarted]);
    ...
    const handleSave = () => {
        ...
        saveGame(saveName, gameState, playTime);
        setSaveName('');
        alert('存檔成功！');
        setActiveTab('load');
    };
    ...
    const handleLoad = () => {
        ...
        const save = loadGame(selectedSave);
        if (save) {
                        ...
                        {activeTab === 'save' ? (
                            <div className="space-y-4">
                                ...
                                <div className="p-4 bg-wuxia-gold/5 border border-wuxia-gold/20 rounded-lg space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-3 text-xs text-white/70">
                                        <div className="flex items-center gap-2">
                                            <MapPin className="w-4 h-4 text-wuxia-gold" />
                                            <span>位置: {gameState.world.location}</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <TrendingUp className="w-4 h-4 text-wuxia-gold" />
                                            <span>等級: {gameState.player.stats.level}</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <Clock className="w-4 h-4 text-wuxia-gold" />
                                            <span>遊戲時間: {formatTime(playTime)}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <button
                                        onClick={handleSave}
                                        className="w-full py-3 bg-gradient-to-r from-wuxia-gold/80 to-wuxia-bronze/80 hover:from-wuxia-gold hover:to-wuxia-bronze border border-wuxia-gold/50 rounded-sm text-black font-bold font-serif tracking-wider transition-all shadow-lg hover:shadow-wuxia-gold/30"
                                    >
                                        保存遊戲
                                    </button>
                                </div>
                                            ...
                                            >
                                                <div className="flex items-start justify-between gap-4">
                                                    <div className="flex-1 space-y-2">
                                                        <div className="flex items-center gap-2">
                                                            <h4 className="font-serif text-wuxia-gold font-bold">
                                                                {save.name}
                                                            </h4>
                                                            {save.name === '自動存檔' && (
                                                                <span className="text-[10px] px-2 py-0.5 bg-cyan-500/20 border border-cyan-500/40 rounded text-cyan-400">
                                                                    AUTO
                                                                </span>
                                                            )}
                                                        </div>
                                                    ...
                                                    <div className="flex gap-1">
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-2 hover:bg-wuxia-crimson/20 rounded-sm transition-colors border border-transparent hover:border-wuxia-crimson/40"
                                                            title="刪除存檔"
                                                        >
