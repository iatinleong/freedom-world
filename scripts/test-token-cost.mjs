...
const P_WORLD = `你是武俠世界的創世說書人，為《自由江湖》生成一個獨一無二的江湖。
...
只回傳 JSON：
{
  ...
  "factions": [{"name":"","alignment":"正道|邪道|中立","philosophy":"","martialStyle":"","personality":"","status":"強盛/式微/擴張中/內亂/蟄伏"}],
  "centralConflict": "核心矛盾一句話"
}
6-8個勢力，各有鮮明立場與功法特色，不得雷同。勢力之間要有具體的恩怨或利益關係。`;

function P_BACKSTORY(worldNarrative, factionNames) {
    const attrFlavors = '膂力：天生神力、力壓千鈞；身法：行動穩健、不快不慢；根骨：根骨奇佳、天生俠骨；悟性：悟性超群、一點即通；定力：心境平和、略有波動；福緣：際遇平平、隨緣而定；魅力：外貌平凡、中規中矩';
    return `你是《自由江湖》的說書人，根據已建立的江湖世界觀，為玩家生成主角的背景故事。
...
只回傳 JSON：
{
  ...
  "relations": {"sect": "所屬門派", "master": "師父名（無則填「無」）"},
  "stateUpdate": {
    "newItems": [{"id":"","name":"","description":"","type":"weapon|armor|consumable|material|book","count":1}],
    "newSkills": [{"name":"","type":"external|internal|light","rank":"基礎","level":"初窺門徑"}],
